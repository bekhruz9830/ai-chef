...
class GeminiService {
  ...
  static const _langNames = {
    ...
    'tr': 'Turkish', 'hi': 'Hindi', 'id': 'Indonesian', 'nl': 'Dutch',
  };

  GeminiService._internal() {
    _chatHistory.add({'role': 'system', 'content': _buildSystemPrompt('en')});
  }

  String _buildSystemPrompt(String langCode) {
    final langName = _langNames[langCode] ?? 'English';
    return 'You are CHEF AI, a culinary expert assistant. '
        'IMPORTANT: Always respond ONLY in $langName, regardless of what language the user writes in. '
  ...
  void setLanguage(String langCode) {
    ...
    if (_chatHistory.isNotEmpty && _chatHistory.first['role'] == 'system') {
      _chatHistory[0] = {'role': 'system', 'content': _buildSystemPrompt(langCode)};
    }
  }
