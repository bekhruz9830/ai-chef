...
class _ShoppingListScreenState extends State<ShoppingListScreen> {
    ...
    if (mounted && data.isNotEmpty) {
      setState(() {
        _items = Map<String, List<String>>.from(
          (data['items'] as Map).map((k, v) => MapEntry(k, List<String>.from(v))),
        );
        _checked = Map<String, bool>.from(data['checked'] ?? {});
      });
    }
  ...
  void _clearChecked() {
    setState(() {
      final checkedKeys = _checked.entries
          .where((e) => e.value)
          .map((e) => e.key)
          .toList();
  ...
  int get _totalCount => _items.values.fold(0, (sum, list) => sum + list.length);
